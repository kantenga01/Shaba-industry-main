...
function Realisations() {
  ...
  const filtered = cat === "Toutes" ? projects : projects.filter((p) => p.category === cat);
            ...
            {filtered.map((p) => (
              <figure key={p.title} className="group relative overflow-hidden">
                ...
                <figcaption className="absolute inset-x-0 bottom-0 bg-ink/85 px-4 py-3 text-ink-foreground">
                  <p className="font-display text-sm font-semibold uppercase">{p.title}</p>
      ...
      <section className="bg-foreground py-14 md:py-20">
        <div className="container-page">
          ...
          <div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {["Chantier industriel", "Atelier impression", "Logistique & convoi"].map((v, i) => (
              <div
                key={v}
                className="relative flex h-48 items-center justify-center overflow-hidden bg-ink text-ink-foreground"
              >
                <img
                  src={imageFor(projects[i]?.service ?? "epi")}
                  alt={v}
                  loading="lazy"
                  width={1200}
                  height={800}
                ...
                <div className="relative flex flex-col items-center gap-3">
                  ...
                  <span className="font-display text-sm uppercase tracking-wide">{v}</span>
                </div>
              </div>
            ))}
          </div>
